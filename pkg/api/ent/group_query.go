...
func (gq *GroupQuery) QueryNamespaces() *NamespaceQuery {
	query := &NamespaceQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withNamespaces: gq.withNamespaces.Clone(),
...
func (gq *GroupQuery) WithNamespaces(opts ...func(*NamespaceQuery)) *GroupQuery {
	query := &NamespaceQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withNamespaces = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context) ([]*Group, error) {
	...
	if query := gq.withNamespaces; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Group, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Namespaces = []*Namespace{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Group)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(group.NamespacesPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, gq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "namespaces": %w`, err)
		}
		query.Where(namespace.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "namespaces" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Namespaces = append(nodes[i].Edges.Namespaces, n)
			}
		}
	}
